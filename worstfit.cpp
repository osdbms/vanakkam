...
int main()
{
int bsize[10], psize[10], bno, pno, flags[10], allocation[10], i, j,max,pos;
for(i = 0; i < 10; i++)
{
flags[i] = 0;
allocation[i] = -1;
}
printf("WORST FIT MEMORY ALLOATION:-----\n");
...
",i+1,allocation[i]+1);
 ...
 printf("\n Process %d is not allocated size of hole after allocation: ",i+1);
 for(j = 0; j < bno; j++)
printf(" %d ", bsize[j]);
 }
return 0;
